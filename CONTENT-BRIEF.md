...
6. **55 MCP tools, tiered.** minimal(3) / lite(8, default) / standard(21) / full(55). Pick a slice, not the kitchen sink.
...
- **97.45% hit@5 on LongMemEval** (stricter recall@5: 93.83%) — one of the hardest memory benchmarks (long multi-session histories, updated/contradicted facts, temporal reasoning). Full 470-question set, NeuroVault's real `recall()` path, 100% on-device embeddings.
