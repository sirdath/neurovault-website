...
- **94% recall on LongMemEval** — one of the hardest memory benchmarks (long multi-session histories, updated/contradicted facts, temporal reasoning). Full 470-question set, NeuroVault's real `recall()` path, 100% on-device embeddings.
- hit@5 ≈ 0.94 (right memory in top 5), hit@10 ≈ 0.98, median query over a 120k-token haystack, zero cloud / no API keys.
