...
- Download: **~28 MB** (macOS DMG) / **~26 MB** (Windows installer) — a fraction of an Electron app's 150 MB+.
...
- **Tauri, not Electron.** WebView2 is Chromium already installed on Windows — no shipping a second browser. The download is tens of MB (≈28 MB on macOS) instead of an Electron app's 150 MB+.
...
The server exposes **55 tools** via a data-driven registry, gated by a **tier** so an agent only loads the slice it needs: `minimal` (3) · `lite` (9, the default) · `standard` (21) · `full` (55). Set it with `NEUROVAULT_MCP_TIER`, `~/.neurovault/mcp_tier.txt`, or Settings → MCP — fewer tools means less tool-definition context the agent pays for up front. Every tool takes an optional `brain` parameter. A representative slice:
